...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    <div
      ...
      className="relative w-full overflow-x-auto rounded-2xl border border-white/45 bg-white/70 shadow-[0_16px_35px_-30px_rgba(76,58,255,0.45)] backdrop-blur-lg"
    >
      <table
        data-slot="table"
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn(
        "p-2 align-top whitespace-nowrap [&:has([role=checkbox])]:pr-0 [&:has([role=checkbox])]:align-middle",
        className
      )}
      {...props}
    />
